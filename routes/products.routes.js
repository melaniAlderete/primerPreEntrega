...
router.get("/", (req, res) => {
  ...
  try {
    if (limit === 0 || !limit) {
      ...
      let limitedArray = arrayOriginal.slice(0, limit);
      res.status(202).json(limitedArray);
    }
  } catch (error) {
...
router.put("/", async (req, res) => {
  ...
  (await ProductManager.updateProduct(id, campo, valorNuevo))
    ...
    : res
        .status(400)
        .json({ error: "Verifique el campo ,no puede modificar el id" });
});
...
router.delete("/:pid", async (req, res) => {
  ...
  (await ProductManager.deleteProduct(pid))
    ...
    : res
        .status(400)
        .json({ error: "El producto no ha sido encontrado por su id" });
});
