...
router.post('/:cid/product/:pid', async (req,res) => {
  ...
  let exists = ProductManager.getProducts().some(el => el.id === Number(pid))
  if (!exists) res.status(404).json({"error":"El producto no se encuentra en la base de datos"})
  else { 
  ...
  : res.status(400).json({error:"Ha ocurrido un error al agregar productos, asegurese de proveer el numero de carrito correcto"})
